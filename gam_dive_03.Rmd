...
`s.wam` は `gam.fit` の中で `bf.call` として定義されたフィッティングのための関数で、 `s.wam` の `wam` は*weighted additive model*の意味のようです。名前からして `gam` の本体のような気がしてきますが、先に言ってしまうとこの関数内で `bakfit` というFortranの関数を呼び出しているので、少なくともRの「 `gam` というライブラリの本体」という意味では合っていると思います。
...
`s.wam` は前回記事で述べたように、第二引数として `y` を受け取ります。
...
上のブロックにおいて、 `gam.match(data)` では以下の結果が返ります：
...
これは `gam.match` という関数の、下記の部分が評価された結果です。
...
```{r}
> match(head(xr), sx, nef + 1)
[1] 4 2 1 1 3 6
```

この結果は `xr` の値が `sx` の何番目に位置するかを表わしていますが、この `sx` は数値をソートしてユニークにしたものなので、順位に変換していることになります。

`gam.match` の結果が `smooth.frame` に代入されているので、この時点で `smooth.frame` は数値の大小関係のみを持っていることになり、また一連の処理で `smooth.frame` は `data.frame` ではなくなるため、ループの２回目以降ではこの処理はスキップされます。
...
の方が説明もあってわかりやすいので、こちらベースに進めたいと思いますが、まずは残りの工程を見ていきます。
