...
では `s.wam` です。これは `gam.fit` の中で `bf.call` として定義されたフィッティングのための関数で、 `s.wam` の `wam` は*weighted additive model*の意味のようです。名前からして `gam` の本体のような気がしてきますが、先に言ってしまうと、この関数内で `bakfit` というFortranの関数を呼び出しているので、少なくともRの「 `gam` というライブラリ」の本体という意味では合っていると思います。
...
o <- match(xr, sx, nef + 1) # 元のデータをラベルに変更する。 +1 は欠損用。
o[is.na(o)] <- nef + 1
```

`gam.match` の結果が `smooth.frame` に代入されているので、この時点で `smooth.frame` は数値の大小関係のみを持っていることになります。また一連の処理で `smooth.frame` は `data.frame` ではなくなるため、ループの２回目以降ではこの処理はスキップされます。
...
さて、この `bakfit` ですが、ソースコードとしてはおそらく[こちら](
