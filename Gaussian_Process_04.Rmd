...
今回は図3.23と3.29に挑戦します。データは、陸上男子100Mについては[こちら](https://www.topendsports.com/sport/athletics/record-100m.htm)から該当する部分を取ってきました。また久保先生の緑本からのデータはサポートサイト(http://chasen.org/~daiti-m/gpbook/data/kubo.dat)から取得しました。
...
ちょっと使っているデータが違っている様子ですが、このまま進めます。

本にしがたい、Xおよびyを平均0、分散1にスケーリングします。`dat`は`data.frame`で持っておきたいので、事前にスケーリングのパラメータは別に取っておきましょう。
...
gen_gpreg_plot <- function(param, x, y) {
   ...
   x <- x * scale_pars[4] + scale_pars[2]
   y <- y * scale_pars[3] + scale_pars[1]
   
   
   test <- seq(1960, 2020, 1)
   ...
   plot(x, y, xlim = c(1960, 2020), ylim = c(9.5, 10.1), type = "n")
   polygon(c(test, rev(test)), c(CI_u, rev(CI_l)), col = 'gray', border = NA)
   points(x, y, xlim = c(1960, 2020), ylim = c(9.5, 10.1), xlab = "x", pch = 4, cex = 2)
   lines(test, Mu, xlim = c(1960, 2020), ylim = c(9.5, 10.1), type = "l", ylab = "")
