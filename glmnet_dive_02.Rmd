...
この関数はそれほど長くないので、いきなり内容の確認に入ります。
```{r}
...
ちなみにここでの `elnet` はコンソールで `elnet` と打っても表示されませんが、C や Fortran で書かれたものではなくて単に `glmnet` からエクスポートされていない関数です。
なので `glmnet:::elnet` で中身を見ることができます。
