...
```{r}
# heatmap.2(s, Rowv = NA, Colv = NA, dendrogram = "none", 
#           trace = "none")
# heatmap.2(t(k) %*% solve(K) %*% k, Rowv = NA, Colv = NA, dendrogram = "none", 
#           trace = "none")
# heatmap.2(Var, Rowv = NA, Colv = NA, dendrogram = "none", 
#           trace = "none")
par(mfrow = c(3, 1), mar = c(3, 3, 1, 1))
plot(cbind(seq(-1, 3.5, 0.05), diag(s)), type = "l", xlab = "", ylab = "diag(Var)")
plot(cbind(seq(-1, 3.5, 0.05), diag(t(k) %*% solve(K) %*% k)), type = "l", xlab = "", ylab = "diag(Var)")
plot(cbind(seq(-1, 3.5, 0.05), diag(Var)), type = "l", xlab = "", ylab = "diag(Var)")
```

`s`は常に一定の値であり、そこから`t(k) %*% solve(K) %*% k)`の値を減じるため、`Var`はちょうど反転した形状になっていますね。`k`は`train$x`と`test`の共分散なので、元のデータが存在する付近では共分散が大きくなっているはずです。確かめてみましょう：
