...
今回は図3.xxと3.yyに挑戦したいのですが、その前に前回定義したカーネルの関数を修正します。
...
上記の関数は入力`x`がスカラであることを前提にしているので、ベクトルに対応させたいです。具体的には`x-x2`としている箇所ですが、入力がベクトルになるのでここでノルムを取るように修正します。同時にベクトル同士をそのまま計算させれば良いので`apply`も外し、また共分散行列は対称行列となるはずなので`t()`も外しました：
