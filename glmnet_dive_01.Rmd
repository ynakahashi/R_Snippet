...
続いて `alpha` のチェックに入ります。
`glmnet` においてこの `alpha` は回帰係数のL1およびL2ノルムそれぞれに対する罰則の割合をコントロールします。
...
まず `y` に対する `drop` ですが、これは length が 1 であるような冗長な次元を落とす関数です。
...
さらに `glmnet.control` で持っているパラメータを渡します。
...
せっかくなので疎行列における数値の格納方法についても見ておきましょう。
以下のように疎行列を作成します：
...
ここで使われている `match.arg` ですが、せっかくなので挙動を確認しておきましょう：
...
https://tutel.me/c/programming/questions/33533045/how+can+i+see+the+source+code+of+fortran+in+r
