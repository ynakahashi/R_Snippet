...
library(tidyverse)
library(ggplot2)
Work_Dir   <- "/Users/nakahashi/Desktop/GitTest/R_Snippet/"
...
```{r}
Var_Num  <- colnames(Dat_Sample)[sapply(Dat_Sample, is_numeric)]
Var_Char <- colnames(Dat_Sample)[sapply(Dat_Sample, is_character)]
```
...
```{r}
Dat_Mat_01 <- as_data_frame(predict(caret::dummyVars(~., data = Dat_Sample),
                                    Dat_Sample))
Dat_Cor_01 <- cor(Dat_Mat, use = "pairwise.complete")
```
...
#### 2. 前処理
```{r}
Dat_Sample %>% arrange(desc(credit_amount)) %>% select(credit_amount)
```
...
Prds <- predict(Res_Step_GLM, newdata = Dat_Test)
```
...
AUC_GLM <- auc(ROC_GLM)
print(AUC_GLM)
GLM_PredictClass <- ifelse(Prds > 0.4229962, 1, 0)
```

機械学習系のアルゴリズムを用いてみる。
アルゴリズムとしてはランダムフォレストとXgboostぐらい。
