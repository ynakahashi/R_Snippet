...
# X, y are 800 by 2 and 800 by 1 data frames respectively.
# They are created in a way such that a linear classifier cannot separate them. 
...
## set min & max for x and y
x_min <- min(X[, 1])-0.2; x_max <- max(X[, 1])+0.2
y_min <- min(X[, 2])-0.2; y_max <- max(X[, 2])+0.2

## Visualize the data
ggplot(data) + 
   geom_point(aes(x = x, y = y, color = as.character(label)), size = 2) + 
...
displayDigit <- function(X){
  m <- matrix(unlist(X), nrow = 28, byrow = TRUE)
  m <- t(apply(m, 2, rev))
  image(m, col=grey.colors(255))
}

## read MNIST data & display
setwd("/Users/nakahashi/Desktop/Rtraining")
train <- read.csv("train.csv", header=TRUE, stringsAsFactors=FALSE)
displayDigit(train[18,-1])

## ?? convert almost 0 value to 0 ??
nzv <- caret::nearZeroVar(train)
nzv.nolabel <- nzv-1

inTrain <- caret::createDataPartition(y=train$label, p=0.7, list=FALSE)
