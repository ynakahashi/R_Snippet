     ...
     *r,maxit,xv,  lmu,ao,ia,kin,rsqo,almo,nlp,jerr)
      ...
      eqs=max(eps,flmin) ! eps と flmin の大きい方を eqs とする。 eps は get_int_parms で eps0 を受け取っており、 1.0d-6 なので 0.00001                                 
...
10351 continue                                                          
      ...
      ! ここは大きなポイント!!!!
      ! 回帰係数を縮小？
      ! g(k)が内積、akが回帰係数（(x'x)^-1 * x'y）、xv が x の二乗和（x'x）とすると、ak * xv = xy になる？
      ! そうすると内積の 2 倍？
      u=g(k)+ak*xv(k)  
      ...
      ! ここで dem を使うのは、Elastic Net なので Lasso の推定値を 1/(1+dem) だけ縮小するためと思われる
