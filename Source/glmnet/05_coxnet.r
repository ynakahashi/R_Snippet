function (x, is.sparse, ix, jx, y, weights, offset, alpha, nobs, 
    ...
    vnames, maxit) 
{
    if (!is.matrix(y) || !all(match(c("time", "status"), dimnames(y)[[2]], 
        0))) 
        stop("Cox model requires a matrix with columns 'time' (>0) and 'status'  (binary) as a response; a 'Surv' object suffices", 
            call. = FALSE)
    ty = as.double(y[, "time"])
    ...
    fit = if (is.sparse) 
        stop("Cox model mot implemented for sparse x in glmnet", 
            call. = FALSE)
    else .Fortran("coxnet", parm = alpha, nobs, nvars, as.double(x), 
        ty, tevent, offset, weights, jd, vp, cl, ne, nx, nlam, 
        flmin, ulam, thresh, maxit, isd, lmu = integer(1), ca = double(nx * 
            nlam), ia = integer(nx), nin = integer(nlam), nulldev = double(1), 
