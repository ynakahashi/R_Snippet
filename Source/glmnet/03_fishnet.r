...
{
    if (any(y < 0)) 
        stop("negative responses encountered;  not permitted for Poisson family")
    ...
    fit = if (is.sparse) 
        .Fortran("spfishnet", parm = alpha, nobs, nvars, x, ix, 
            jx, y, offset, weights, jd, vp, cl, ne, nx, nlam, 
            flmin, ulam, thresh, isd, intr, maxit, lmu = integer(1), 
            ...
            PACKAGE = "glmnet")
    else .Fortran("fishnet", parm = alpha, nobs, nvars, as.double(x), 
        y, offset, weights, jd, vp, cl, ne, nx, nlam, flmin, 
        ulam, thresh, isd, intr, maxit, lmu = integer(1), a0 = double(nlam), 
