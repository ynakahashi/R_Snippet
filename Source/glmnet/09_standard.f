      ...
      ! まずは y      
      ym=0.0 ! 切片が 0 である指定だから 0                                                           
      y=v*y ! v は sqrt(w), w = w/sum(w), w は weights なので観測値に対する重み   
      ! dot_product は内積                                                         
      ys=sqrt(dot_product(y,y)-dot_product(v,y)**2) ! y の二乗の重み付き平均から重み付き平均の二乗を引き（分散）、その平方根を取る（SD）
      y=y/ys ! 標準化
      ...
      xbq=dot_product(v,x(:,j))**2 ! 重み付き平均の二乗
      vc=xv(j)-xbq ! 重み付き分散                                                     
      xs(j)=sqrt(vc) ! 重み付き標準偏差。ys と対応している。
      x(:,j)=x(:,j)/xs(j) ! 標準化。 y/ys と対応している。
